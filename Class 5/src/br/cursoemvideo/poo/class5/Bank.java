...
public class Bank {
	...
	public Bank(String responsavel) {
		...
		setSaldo(0f);
		setStatus(false);
		System.out.println("\nParabéns " + responsavel + " Você acaba de abrir uma conta no banco NILBA!!!");
	...
	public void depositar(float valor) {
		if (isStatus()) {
			...
			System.out.println("***Sua conta se encontra com sutuação encerrada***\nNão é possível depositar!");
		}
	}
	...
	public void sacar(float valor) {
		if (isStatus() && getSaldo() >= valor) {
			...
			System.out.println("Você está com saldo insuficiente para saque!!!");
		}
	}
